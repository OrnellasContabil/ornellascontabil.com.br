...
function ext(n) {
  ...
  if (n < 1000000000) {
    ...
    const r = milhoes === 1 ? 'um milhão' : ext(milhoes) + ' milhões';
    if (!resto) return r;
    // "e" só antes do resto quando ele é < 100 ou múltiplo de 100 (regra gramatical
    // padrão de números por extenso em português: "e" liga a última unidade/centena).
    const liga = (resto < 100 || resto % 100 === 0) ? ' e ' : ', ';
    return r + liga + ext(resto);
  }
  const bilhoes = Math.floor(n / 1000000000);
  const resto = n % 1000000000;
  const r = bilhoes === 1 ? 'um bilhão' : ext(bilhoes) + ' bilhões';
  if (!resto) return r;
  const liga = (resto < 100 || resto % 100 === 0) ? ' e ' : ', ';
  return r + liga + ext(resto);
}
...
function valorExtenso(s) {
  ...
  const base = ext(num);
  // "de" só aparece quando o número termina exatamente em milhão(ões)/bilhão(ões),
  // sem resto de centena/mil (ex: "um milhão de reais", mas "um milhão e cem reais").
  const terminaEmMilhaoOuBilhao = /(milhão|milhões|bilhão|bilhões)$/.test(base);
  return base + (terminaEmMilhaoOuBilhao ? ' de ' : ' ') + (num !== 1 ? 'reais' : 'real');
}
...
function qualificacao(s) {
  ...
  const prof = s.prof || '';
  // Aceita tanto data ISO (YYYY-MM-DD, vinda direto do formulário) quanto já
  // formatada — sempre exibe no padrão dia/mês/ano no documento final.
  const nasc = (s.nasc && /^\d{4}-\d{2}-\d{2}/.test(s.nasc)) ? fmtDataCurta(s.nasc) : (s.nasc || '');
  const nat = s.nat || '';
  const cpf = s.cpf || '';
  const rg = s.rg || '';
...
function admTexto(socios, ehEI) {
  let adms = socios.filter(s => (s.papel || '').toLowerCase().includes('administrador'));
  if (!adms.length) adms = socios.slice(0, 1);
  if (ehEI) {
    const s = adms[0] || socios[0] || {};
    const fem = (s.genero || 'M') === 'F';
    return `pel${fem ? 'a' : 'o'} próprio${fem ? 'a' : ''} titular ${(s.nome || '').toUpperCase()}`;
  }
  if (adms.length === 1) {
    const s = adms[0];
    const fem = (s.genero || 'M') === 'F';
...
// Nome jurídico completo de cada tipo (sigla -> nome por extenso), usado em textos
// explicativos de cláusulas (ex: transformação de tipo jurídico). A sigla continua
// sendo o valor salvo/usado na denominação da empresa ("EMPRESA X LTDA") — esse
// mapeamento NUNCA substitui a sigla ali, só fornece o nome completo quando o texto
// jurídico precisa dele por extenso. Espelha LEG_TIPOS_JUR_NOME_COMPLETO do index.html.
const TIPOS_JUR_NOME_COMPLETO = {
...
function nomeJuridicoCompleto(sigla) {
  return TIPOS_JUR_NOME_COMPLETO[sigla] || sigla || 'tipo jurídico não informado';
}
// Identifica se um tipo jurídico é "Empresa Individual" em qualquer uma de suas
// variações usadas no sistema (Empresário Individual ou EIRELI), para a regra
// especial da cláusula de transformação (art. 980 do Código Civil). Normaliza
// acentos/caixa/espaços antes de comparar, para não depender de o valor salvo
// no processo bater byte a byte com a string exata da lista de opções.
function normalizarTipoJur(tipo) {
  return String(tipo || '')
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .trim().toLowerCase();
}
function ehEmpresaIndividual(tipo) {
  const n = normalizarTipoJur(tipo);
  return n === 'empresario individual' || n === 'eireli';
}
// Troca "sócio/sócio(a)/sócios" por "titular/titular/titulares" preservando
// maiúscula inicial, para uso nos documentos de Empresário Individual (onde
// não existe sociedade nem quadro de sócios, apenas um titular).
function termoSocio(txt) {
  if (!txt) return txt;
  const cap = (orig, novo) => (orig[0] === orig[0].toUpperCase() ? novo.charAt(0).toUpperCase() + novo.slice(1) : novo);
  return String(txt)
    .replace(/sócio\(a\)s/gi, m => cap(m, 'titulares'))
    .replace(/sócios/gi, m => cap(m, 'titulares'))
    .replace(/sócio\(a\)/gi, m => cap(m, 'titular'))
    .replace(/sócio/gi, m => cap(m, 'titular'));
}
...
function drawTitleBlock(doc, tipoLabel, nomeEmp, tipoJur, topMargin, ehEI) {
  ensureSpace(doc, 60);
  if (tipoLabel) {
    doc.x = LEFT;
  ...
  doc.text(`${nomeEmp}${ehEI ? '' : ' ' + tipoJur}`.toUpperCase(), { width: CW, align: 'center' });
...
function drawQuotaTable(doc, socios, capitalTotal, topMargin, ehEI) {
  ...
  const header = [ehEI ? 'TITULAR' : 'SÓCIO', 'QUOTAS', 'VALOR R$', '%'];
...
function drawSignBlock(doc, socios, adv, topMargin, dataLocalTexto, ehEI) {
  ...
  socios.forEach((s, i) => {
    ...
    blockLines.push({ text: ehEI ? termoSocio(s.papel || 'Administrador(a) e Sócio(a)') : (s.papel || 'Sócio(a)'), bold: false, gapBefore: 1 });
...
function escreverCorpoContratoSocial(doc, proc, topMargin, opts = {}) {
  ...
  // Empresário Individual: não há "sociedade" nem quadro de "sócios" — apenas um titular.
  const ehEI = ehEmpresaIndividual(tipoJur);
  const termoEnte = ehEI ? 'empresa' : 'sociedade';
  const doSocioOuTitular = ehEI ? 'do titular' : sp.dos_socios;
  ...
  const preambuloTxt = ehEI
    ? quals.join(', e ') + ' resolve constituir, como empresário individual, que será regida, pelas seguintes cláusulas e condições:'
    : quals.join(', e ') +
  ...
  // Título
  drawTitleBlock(doc, tituloPrincipal, nomeEmp, tipoJur, topMargin, ehEI);
  ...
  writeBoldLeadParagraph(doc, clausula.next() + ' –',
    `A ${termoEnte} girará sob a denominação ${nomeEmp}${ehEI ? '' : ' ' + tipoJur}.`, topMargin);
  ...
  writeBoldLeadParagraph(doc, clausula.next() + ' –',
    `A ${termoEnte} tem sua sede na ${enderecoTxt}, podendo abrir ou encerrar filiais em ` +
    `qualquer parte do território nacional por deliberação ${doSocioOuTitular}.`, topMargin);

  writeBoldLeadParagraph(doc, clausula.next() + ' –',
    `A ${termoEnte} tem por objeto as seguintes atividades: ${objeto}.`, topMargin);

  writeBoldLeadParagraph(doc, clausula.next() + ' –',
    `A ${termoEnte} iniciará suas atividades na data do arquivamento deste ato e seu prazo de duração é por tempo indeterminado.`,
    topMargin);
  ...
  drawQuotaTable(doc, socios, capitalN, topMargin, ehEI);
  ...
  writeBoldLeadParagraph(doc, 'PARÁGRAFO PRIMEIRO –',
    ...
    `sem o consentimento ${ehEI ? 'do titular' : sp.dos_socios}, a quem fica assegurado, em igualdade de ` +
  ...
  writeBoldLeadParagraph(doc, 'PARÁGRAFO SEGUNDO –',
    `A responsabilidade ${ehEI ? 'do titular é restrita' : 'de cada sócio é restrita'} ao valor de suas quotas, mas ` +
    `${ehEI ? 'responde' : 'todos respondem'} solidariamente pela integralização do capital social, conforme art. 1.052, CC/2002.`,
    topMargin);

  // Administração
  writeSectionTitle(doc, 'ADMINISTRAÇÃO', topMargin);
  writeBoldLeadParagraph(doc, clausula.next() + ' –',
    `A ${termoEnte} será administrada ${admTexto(socios, ehEI)}, que exercerá a função de ` +
    'administrador, com poderes de gestão, representação ativa e passiva da ' +
    `${termoEnte}, podendo praticar todos os atos necessários ao funcionamento regular ` +
  ...
  writeBoldLeadParagraph(doc, clausula.next() + ' –',
    ...
    `distribuídos proporcionalmente às quotas de participação ${ehEI ? 'do titular' : 'de cada sócio'} ou ` +
    `destinados à formação de reservas, conforme deliberação ${doSocioOuTitular}.`, topMargin);

  // Dissolução e liquidação
  writeSectionTitle(doc, 'DA DISSOLUÇÃO E LIQUIDAÇÃO', topMargin);
  writeBoldLeadParagraph(doc, clausula.next() + ' –',
    `A ${termoEnte} poderá ser dissolvida por deliberação ${doSocioOuTitular} ou pelas ` +
  ...
  writeBoldLeadParagraph(doc, clausula.next() + ' –',
    `${ehEI ? 'O titular declara' : 'O sócio ' + sp.declaram} que a ${termoEnte} preenche os requisitos estabelecidos ` +
  ...
  writeBoldLeadParagraph(doc, clausula.next() + ' –',
    `${ehEI ? 'O titular declara' : 'O sócio ' + sp.declaram}, sob as penas da lei, que não ${ehEI ? 'está impedido' : 'estão impedidos'} de ` +
    `exercer atividade mercantil ou de administrar ${ehEI ? 'empresa individual' : 'sociedade empresária'}, e que não ` +
    `${ehEI ? 'foi condenado' : 'foram condenados'} a penas que vedem, ainda que temporariamente, o acesso a ` +
  ...
  if (!omitirAssinaturas) {
    drawSignBlock(doc, socios, adv, topMargin, dataLocalTexto, ehEI);
  }
...
function valorExtensoComCentavos(v) {
  ...
  const extInteiro = ext(inteiro);
  const terminaEmMilhaoOuBilhao = /(milhão|milhões|bilhão|bilhões)$/.test(extInteiro);
  let texto = extInteiro + (terminaEmMilhaoOuBilhao ? ' de ' : ' ') + moeda;
  if (centavos > 0) {
...
function calcularSnapshotPosAlteracao(proc) {
  ...
  if (sub.socio) {
    if (ad.socioTipo === 'cessao') {
      ...
      // Guarda a referência do objeto cessionário ANTES de qualquer filtro que possa
      // remover itens do array — remover por índice desloca as posições de todos os
      // itens depois dele, então um índice salvo anteriormente ficaria apontando para
      // a pessoa errada se resolvido depois do filtro. Referência de objeto não tem
      // esse problema: ela continua válida mesmo que a posição no array mude.
      const cessionarioExistenteRef = (ad.cessCessionarioTipo === 'existente' && ad.cessCessionarioIdx !== '' && ad.cessCessionarioIdx != null)
        ? snap.socios_dados[parseInt(ad.cessCessionarioIdx, 10)]
        : null;
      ...
      } else if (cessionarioExistenteRef) {
        const atuais = parseInt(String(cessionarioExistenteRef.quotas || '0').replace(/\D/g, ''), 10) || 0;
        cessionarioExistenteRef.quotas = String(atuais + quotasCedidas);
      }
    ...
    } else if (ad.socioTipo === 'entrada' && ad.entradasNovas && ad.entradasNovas.length) {
      const entradas = ad.entradasNovas;
      const quotasPorEntrada = entradas.map(e => parseInt(String(e.quotas || '0').replace(/\D/g, ''), 10) || 0);
      const totalQuotasNovas = quotasPorEntrada.reduce((a, b) => a + b, 0);
      if (!sub.capital) {
        ...
        if (totalAntesNum > 0) {
          snap.socios_dados.forEach(s => {
            const atuais = parseInt(String(s.quotas || '0').replace(/\D/g, ''), 10) || 0;
            const reducao = Math.round(atuais / totalAntesNum * totalQuotasNovas);
            s.quotas = String(Math.max(0, atuais - reducao));
          });
        }
      }
      entradas.forEach((e, idx) => {
        snap.socios_dados.push({ ...normalizarPessoaParaQualificacao(e), quotas: String(quotasPorEntrada[idx]), papel: 'Sócio(a) não administrador(a)' });
      });
    } else if (ad.socioTipo === 'saida' && ad.saidaSociosIdx && ad.saidaSociosIdx.length) {
      const idxList = ad.saidaSociosIdx;
      const quotasSaida = idxList.reduce((acc, idx) => {
        const s = snap.socios_dados[idx];
        return acc + (s ? (parseInt(String(s.quotas || '0').replace(/\D/g, ''), 10) || 0) : 0);
      }, 0);
      snap.socios_dados = snap.socios_dados.filter((s, i) => !idxList.includes(i));
...
function escreverClausulasAlteracao(doc, proc, topMargin, snapshot) {
  ...
  if (sub.tipo_juridico && ad.tjPara) {
    ...
    // Regra especial: transformação de Empresa Individual (Empresário Individual ou
    // EIRELI) em Sociedade (qualquer tipo societário) usa o texto específico do
    // art. 980 do Código Civil, em vez do texto genérico de transformação.
    const eraEmpresaIndividual = ehEmpresaIndividual(ad.tjDe);
    const novoEhSociedade = !ehEmpresaIndividual(ad.tjPara) && ad.tjPara !== 'MEI';
    let textoTransformacao;
    if (eraEmpresaIndividual && novoEhSociedade) {
      textoTransformacao = `Fica transformada a natureza jurídica desta Empresa Individual, em ${nomeJuridicoCompleto(ad.tjPara)}, conforme faculta a Lei nº 10.406/02, artigo 980, que doravante se regerá com sub-rogação de todos os direitos e obrigações pertinentes ao tipo jurídico ora transformado.${mot}`;
    } else {
      textoTransformacao = `A sociedade passa a adotar o tipo jurídico ${ad.tjPara}, mantendo-se inalteradas as demais características da pessoa jurídica.${mot}`;
    }
    escreverBloco('DO TIPO JURÍDICO', [textoTransformacao]);
  }
  ...
  if (sub.socio) {
    if (ad.socioTipo === 'cessao') {
      ...
      ad.entradasNovas.forEach(entrada => {
        const quotasNovo = parseInt(String(entrada.quotas || '0').replace(/\D/g, ''), 10) || 0;
        escreverBloco('DA ADMISSÃO DO SÓCIO', [
          `Ingressa na sociedade, na qualidade de sócio(a), ${qualificacaoPessoaAlteracao(entrada)}, subscrevendo ` +
          `${quotasNovo ? `${fmtNum(quotasNovo)} (${ext(quotasNovo)}) quotas` : 'quotas'} do capital social, no valor nominal ` +
          `de R$ 1,00 (um real) cada${entrada.obs ? `, ${entrada.obs}` : ''}.`,
        ]);
      });
      if (!sub.capital) {
        const maisDeUm = ad.entradasNovas.length > 1;
        escreverBlocoComTabela('DO CAPITAL SOCIAL', [
          `Em razão da admissão d${maisDeUm ? 'os novos sócios' : 'o novo sócio'} acima, as quotas dos demais sócios ficam proporcionalmente diluídas, ` +
          'permanecendo o capital social no mesmo valor total, conforme nova composição do quadro societário a seguir.',
        ]);
      }
    } else if (ad.socioTipo === 'saida' && ad.saidaSociosIdx && ad.saidaSociosIdx.length) {
      const saindoLista = ad.saidaSociosIdx.map(idx => socios[idx]).filter(Boolean);
      const nomesSaida = saindoLista.map(s => (s.nome || '').toUpperCase()).join(', ');
      const maisDeUm = saindoLista.length > 1;
      escreverBloco('DA RETIRADA DO SÓCIO', [
        `${maisDeUm ? 'Os sócios' : 'O(A) sócio(a)'} ${nomesSaida}, já qualificado(s) no preâmbulo, RETIRA(M)-SE definitivamente do quadro societário da ` +
        `sociedade, nada mais tendo a reclamar da sociedade ou dos demais sócios a qualquer título, outorgando a mais ` +
        `plena quitação por todos os direitos decorrentes de sua(s) participação(ões) societária(s).`,
      ]);
      if (!sub.capital) {
        const textoDestino = ad.saidaDestino === 'reduzir'
          ? 'o capital social é reduzido proporcionalmente ao valor das quotas do(s) sócio(s) retirante(s)'
          : 'as quotas do(s) sócio(s) retirante(s) são redistribuídas entre os sócios remanescentes, permanecendo o capital social no mesmo valor total';
        escreverBlocoComTabela('DO CAPITAL SOCIAL', [
...
// ════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════
// Geração principal do Distrato Social (Baixa/Encerramento)
// ════════════════════════════════════════════════════════════
function gerarDistratoSocial(proc, logoBuffer, comTimbrado) {
  return new Promise((resolve, reject) => {
    try {
      const socios = (proc.socios_dados && proc.socios_dados.length) ? proc.socios_dados : [{}];
      const nomeEmp = proc.nome || '';
      ...
      const cidade = proc.cidade || 'Niterói';
      const uf = proc.uf || 'RJ';
      const dtEnc = proc.dtEncerramento ? fmtDataExtenso(proc.dtEncerramento) : 'data não informada';
      const hoje = fmtDataExtenso(null);
      const adv = (proc.porte === 'Demais' && proc.advogado) ? proc.advogado : null;

      const sitAtivoMap = {
        positivo: 'existindo saldo positivo a ser distribuído entre os sócios na proporção de suas respectivas participações no capital social',
        zerado: 'não havendo saldo a distribuir nem passivo a quitar, estando a sociedade com balanço zerado',
        negativo: 'existindo passivo a ser quitado pelos sócios, na proporção de suas respectivas participações no capital social',
      };
      const sitTexto = sitAtivoMap[proc.sitAtivo] || 'situação do ativo/passivo a ser apurada em balanço de encerramento';

      // Endereço da sede (mesma robustez usada na Alteração Contratual)
      const sede = proc.enderecoSede || {};
      const sedeTemDados = sede.logradouro || sede.bairro || sede.cidade || sede.cep;
      let enderecoSedeAtual;
      if (sedeTemDados) {
        const partes = [];
        if (sede.logradouro) partes.push(sede.logradouro + (sede.numero ? `, nº ${sede.numero}` : ''));
        if (sede.complemento) partes.push(sede.complemento);
        if (sede.bairro) partes.push(sede.bairro);
        if (sede.cidade) partes.push(`${sede.cidade}/${sede.uf || ''}`);
        enderecoSedeAtual = partes.join(', ');
        if (sede.cep) enderecoSedeAtual += `, CEP ${sede.cep}`;
      } else {
        enderecoSedeAtual = proc.endereco || 'endereço não informado';
      }

      // Registro variável (Junta Comercial / Cartório), igual à Alteração Contratual
      const registroTexto = textoOrgaoRegistro(proc.regDados, '');
      ...
      // Título
      drawTitleBlock(doc, 'DISTRATO SOCIAL', nomeEmp, tipoJur, topMargin);
      doc.x = LEFT;
      doc.font('Times-Roman').fontSize(10);
      doc.text(`NIRE: ${nire || 'não informado'} | CNPJ: ${cnpj}`, { width: CW, align: 'center' });
      doc.moveDown(1);
      ...
      // Preâmbulo
      writeBodyParagraph(doc,
        `Pelo presente instrumento particular de Distrato Social, os sócios abaixo identificados e ` +
        `qualificados, da empresa ${nomeEmp} ${tipoJur}, CNPJ nº ${cnpj}, com sede em ${enderecoSedeAtual}, ` +
        `${registroTexto}, resolvem, em comum acordo, dissolver e encerrar definitivamente as atividades ` +
        'da sociedade, nos termos que se seguem:', topMargin);

      // Identificação e qualificação dos sócios
      writeSectionTitle(doc, 'IDENTIFICAÇÃO E QUALIFICAÇÃO DOS SÓCIOS', topMargin);
      socios.forEach((s, i) => {
        const letra = String.fromCharCode(65 + i);
        writeBodyParagraph(doc, `SÓCIO ${letra}: ${qualificacao(s)}.`, topMargin);
      });

      // Da dissolução
      writeSectionTitle(doc, 'DA DISSOLUÇÃO', topMargin);
      writeBoldLeadParagraph(doc, clausula.next() + ' -',
        `Os sócios deliberam, por unanimidade, a dissolução total e o encerramento definitivo das ` +
        `atividades da sociedade ${nomeEmp} ${tipoJur}, CNPJ nº ${cnpj}, nos termos dos artigos 1.033 e ` +
        'seguintes do Código Civil Brasileiro.', topMargin);

      // Da data de encerramento
      writeSectionTitle(doc, 'DA DATA DE ENCERRAMENTO', topMargin);
      writeBoldLeadParagraph(doc, clausula.next() + ' -',
        `Fica estabelecida como data de encerramento das atividades da sociedade o dia ${dtEnc}, a partir ` +
        'da qual a empresa não mais exercerá qualquer atividade empresarial ou comercial.', topMargin);

      // Da situação do ativo e do passivo
      writeSectionTitle(doc, 'DA SITUAÇÃO DO ATIVO E DO PASSIVO', topMargin);
      writeBoldLeadParagraph(doc, clausula.next() + ' -',
        `Os sócios declaram que, na data do presente distrato, ${sitTexto}. Os sócios respondem ` +
        'solidariamente pelas obrigações trabalhistas, previdenciárias, fiscais e demais encargos da ' +
        'sociedade até a data de encerramento.', topMargin);

      // Da liquidação
      writeSectionTitle(doc, 'DA LIQUIDAÇÃO', topMargin);
      writeBoldLeadParagraph(doc, clausula.next() + ' -',
        'Fica nomeado liquidante o(a) sócio(a) administrador(a), com poderes para praticar todos os atos ' +
        'necessários à liquidação da sociedade, incluindo a realização do ativo, o pagamento do passivo e ' +
        'a partilha do saldo remanescente entre os sócios, caso existente.', topMargin);

      // Da baixa de registros
      writeSectionTitle(doc, 'DA BAIXA DE REGISTROS', topMargin);
      writeBoldLeadParagraph(doc, clausula.next() + ' -',
        'Os sócios se comprometem a providenciar, no menor prazo possível, a baixa de todos os registros ' +
        'da empresa perante os órgãos competentes, incluindo: Junta Comercial/Cartório, Receita Federal ' +
        '(CNPJ), Secretaria de Estado de Fazenda (SEFAZ), Município, INSS, Caixa Econômica Federal (FGTS) ' +
        'e demais órgãos aplicáveis.', topMargin);

      // Da responsabilidade pós-dissolução
      writeSectionTitle(doc, 'DA RESPONSABILIDADE PÓS-DISSOLUÇÃO', topMargin);
      writeBoldLeadParagraph(doc, clausula.next() + ' -',
        'Os sócios declaram, sob as penas da lei, que não há qualquer passivo ou obrigação pendente de ' +
        'natureza trabalhista, fiscal ou previdenciária que não esteja devidamente previsto e tratado ' +
        'neste instrumento.', topMargin);

      // Do foro
      writeSectionTitle(doc, 'DO FORO', topMargin);
      writeBoldLeadParagraph(doc, clausula.next() + ' -',
        `Fica eleito o foro da Comarca de ${foro} para dirimir quaisquer litígios decorrentes do presente ` +
        'distrato.', topMargin);

      // Data e assinaturas (sempre juntas, ver drawSignBlock)
      const dataLocalTexto = `${cidade}/${uf}, ${hoje}.`;
...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (tipo === 'alteracao') {
      ...
      pdfBuffer = await gerarDistratoSocial(proc, logoBuffer, comTimbrado);
      sufixo = 'Distrato_Social';
    } else {
      return {
        statusCode: 400,
        headers: corsHeaders,
