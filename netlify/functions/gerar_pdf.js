...
function textoOrgaoRegistro(altDados) {
  const ad = altDados || {};
  ...
  const numTexto = ad.regNumero ? `, sob ${numLabel} nº ${ad.regNumero}` : '';
  ...
  if (!ad.regNome && !ad.regNumero && !ad.regProtocolo) {
    return 'arquivada na Junta Comercial/Cartório competente';
  }
  return `arquivada na ${orgaoNome}${numTexto}${protocoloTexto}`;
}

function montarClausulasAlteracao(proc) {
  const subtipos = proc.subtipos || {};
  const ad = proc.altDados || {};
  const clausulas = [];

  if (subtipos.capital) {
    const de = ad.capitalDe ? ` (antes: R$ ${ad.capitalDe})` : '';
    const obs = ad.capitalObs ? ` — ${ad.capitalObs}` : '';
    const valorPara = ad.capitalPara || fmtCapitalBRL(proc.capital).replace(/^R\$\s*/, '') || 'valor a definir';
    clausulas.push({
      titulo: 'CLÁUSULA DE MODIFICAÇÃO – CAPITAL SOCIAL:',
      texto: `Altera-se o capital social da sociedade para R$ ${valorPara}${de}${obs}.`,
    });
  }

  if (subtipos.socio) {
    const tipoSocioMap = {
      entrada: 'com a entrada de novo(a) sócio(a)',
      saida: 'com a saída de sócio(a)',
      cessao: 'mediante cessão de quotas entre os sócios',
      redistribuicao: 'mediante redistribuição das quotas sociais',
    };
    const tipoTexto = tipoSocioMap[ad.socioTipo] || 'nos termos acordados entre as partes';
    const envolvidos = ad.socioNomes ? ` Sócio(s) envolvido(s): ${ad.socioNomes}.` : '';
    const desc = ad.socioDesc ? ` ${ad.socioDesc}` : '';
    clausulas.push({
      titulo: 'CLÁUSULA DE MODIFICAÇÃO – QUADRO SOCIAL:',
      texto: `Altera-se o quadro societário da empresa ${tipoTexto}, conforme deliberação ` +
        `unânime dos sócios e documento arquivado em conjunto.${envolvidos}${desc}`,
    });
  }

  if (subtipos.endereco) {
    const endNovo = ad.endPara || proc.endereco || 'endereço não informado';
    const mun = ad.endMunicipio ? `, ${ad.endMunicipio}` : '';
    const uf = ad.endUf ? `/${ad.endUf}` : '';
    const de = ad.endDe ? ` (antes: ${ad.endDe})` : '';
    clausulas.push({
      titulo: 'CLÁUSULA DE MODIFICAÇÃO – SEDE:',
      texto: `Altera-se o endereço da sede social da empresa para: ${endNovo}${mun}${uf}${de}.`,
    });
  }

  if (subtipos.atividades) {
    const objNovo = ad.objPara || proc.objeto || 'objeto social não informado';
    const cnaes = ad.objCnaes ? ` Alterações de CNAE: ${ad.objCnaes}.` : '';
    clausulas.push({
      titulo: 'CLÁUSULA DE MODIFICAÇÃO – OBJETO SOCIAL:',
      texto: `Altera-se o objeto social da empresa, que passará a ser: ${objNovo}.${cnaes}`,
    });
  }

  if (subtipos.tipo_juridico) {
    const de = ad.tjDe ? ` (antes: ${ad.tjDe})` : '';
    const mot = ad.tjMotivo ? ` Fundamento: ${ad.tjMotivo}.` : '';
    clausulas.push({
      titulo: 'CLÁUSULA DE MODIFICAÇÃO – TIPO JURÍDICO:',
      texto: `Altera-se o tipo jurídico da sociedade para ${ad.tjPara || proc.tipoJuridico || 'LTDA'}${de}.${mot}`,
    });
  }

  if (subtipos.nome) {
    const nomeNovo = ad.nomePara || proc.nome || '';
    const tipoJur = proc.tipoJuridico || 'LTDA';
    const de = ad.nomeDe ? ` (antes: ${ad.nomeDe})` : '';
    const fantasia = ad.nomeFantasia ? ` Nome fantasia: ${ad.nomeFantasia}.` : '';
    const consultaMap = {
      sim: 'Consulta prévia de nome aprovada pela Junta Comercial/Cartório.',
      pendente: 'Consulta prévia pendente junto à Junta Comercial/Cartório.',
      nao: '',
    };
    const consulta = ad.nomeConsulta ? ` ${consultaMap[ad.nomeConsulta] || ''}` : '';
    clausulas.push({
      titulo: 'CLÁUSULA DE MODIFICAÇÃO – DENOMINAÇÃO SOCIAL:',
      texto: `Altera-se a denominação social da empresa para: ${nomeNovo} ${tipoJur}${de}.${fantasia}${consulta}`,
    });
  }

  if (subtipos.administrador) {
    const admTipoMap = {
    ...
    clausulas.push({
      titulo: 'CLÁUSULA DE MODIFICAÇÃO – ADMINISTRAÇÃO:',
      texto: `${tipoTexto}${de}${para}, conforme deliberação unânime dos sócios.${mandato}${poderes}`,
    });
  }
  ...
  if (clausulas.length === 0) {
    clausulas.push({
      titulo: '',
      texto: 'As cláusulas a serem alteradas serão especificadas conforme deliberação dos sócios.',
    });
  }
...
// ════════════════════════════════════════════════════════════
// Geração principal da Alteração Contratual
// ════════════════════════════════════════════════════════════
function gerarAlteracaoContratual(proc, logoBuffer, comTimbrado) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const endereco = proc.endereco || 'endereço não informado';
      const foro = proc.foro || 'Niterói/RJ';
      const dataDoc = proc.dataContrato ? fmtDataExtenso(proc.dataContrato) : fmtDataExtenso(null);
      const cidade = proc.cidade || 'Niterói';
      const uf = proc.uf || 'RJ';
      const adv = (proc.porte === 'Demais' && proc.advogado) ? proc.advogado : null;

      const clausulas = montarClausulasAlteracao(proc);
      ...
      // Título
      drawTitleBlock(doc, numAlt.toUpperCase() + ' AO CONTRATO SOCIAL', nomeEmp, tipoJur, topMargin);

      // Preâmbulo
      writeBodyParagraph(doc,
        `Pelo presente instrumento particular de ${numAlt} ao Contrato Social, os sócios ` +
        `abaixo identificados e qualificados, da empresa ${nomeEmp} ${tipoJur}, CNPJ nº ${cnpj}, ` +
        `com sede em ${endereco}, ${textoOrgaoRegistro(proc.altDados)}, resolvem, ` +
        'de comum acordo, promover as seguintes alterações no contrato social:', topMargin);

      // I — Das Partes
      writeSectionTitle(doc, 'I – DAS PARTES', topMargin);
      socios.forEach((s, i) => {
        const letra = String.fromCharCode(65 + i);
        const ec = s.ecivil ? estadoCivilComRegime(s) : 'estado civil não informado';
        const prof = s.prof ? `${s.prof}, ` : '';
        writeBodyParagraph(doc,
          `SÓCIO ${letra}: ${s.nome || 'Nome não informado'}, ${s.nac || 'brasileiro(a)'}, ` +
          `${ec}, ${prof}portador(a) do CPF nº ${s.cpf || '000.000.000-00'} e ` +
          `RG nº ${s.rg || '0000000'}.`, topMargin);
      });

      // II — Das Alterações
      writeSectionTitle(doc, 'II – DAS ALTERAÇÕES', topMargin);
      clausulas.forEach(c => {
        if (c.titulo) {
          writeBoldLeadParagraph(doc, c.titulo, c.texto, topMargin);
        } else {
          writeBodyParagraph(doc, c.texto, topMargin);
        }
      });

      // III — Das Disposições Inalteradas
      writeSectionTitle(doc, 'III – DAS DISPOSIÇÕES INALTERADAS', topMargin);
      writeBodyParagraph(doc,
        'Permanecem em pleno vigor todas as demais cláusulas e condições do Contrato Social ' +
        'original e de suas anteriores alterações, que não tenham sido expressamente ' +
        'modificadas pelo presente instrumento.', topMargin);

      // IV — Do Foro
      writeSectionTitle(doc, 'IV – DO FORO', topMargin);
      writeBodyParagraph(doc,
        `Fica eleito o foro da Comarca de ${foro} para dirimir quaisquer litígios decorrentes ` +
        'do presente instrumento.', topMargin);
