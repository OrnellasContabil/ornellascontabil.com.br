...
function escreverClausulasAlteracao(doc, proc, topMargin, snapshot) {
  ...
  if (sub.capital) {
    const capitalAtualFmt = ad.capitalDe ? `R$ ${ad.capitalDe}` : fmtCapitalBRL(proc.capital);
    const capitalNovoFmt = ad.capitalPara ? `R$ ${ad.capitalPara}` : capitalAtualFmt;
    const obs = ad.capitalObs ? ` ${ad.capitalObs}.` : '';
    escreverBlocoComTabela('DO CAPITAL SOCIAL', [
      `O capital social que é de ${capitalAtualFmt}, passa a ser de ${capitalNovoFmt}, conforme nova composição do quadro societário a seguir.${obs}`,
    ]);
  }
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      writeBodyParagraph(doc,
        `${unicoOuComponentes} ${nomeEmp} ${tipoJur}, devidamente registrada na ${registroOrgaoSemPrefixo}, ` +
